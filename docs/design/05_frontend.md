...
- ゲーム前: 初期画面 → 主催/参加 → ルームIDで合流 → 4 人揃って開始
...
ゲーム開始までの導線。**ルームID**(UUID、ハイフンなし 32 文字 hex)を介して主催側と参加側が出会う方式。
...
stateDiagram-v2
    ...
    初期画面 --> 主催画面: 主催ボタン
    初期画面 --> 参加前画面: 参加ボタン
    参加前画面 --> 参加後画面: ルームID送信(成功)
    参加前画面 --> 参加前画面: 不正なルームID(エラー表示)
    主催画面 --> ゲーム画面: 4人揃って開始
    参加後画面 --> ゲーム画面: 4人揃って開始
```
...
- 主催画面とほぼ同レイアウト(`view.phase === "lobby"`)
- ルームIDと参加プレイヤー一覧を表示
- ホストかどうかは `view.self.id === view.hostPlayerId` で判定
  - ホスト: 「ゲーム開始」ボタン表示・押下可
  - ゲスト: ボタン非表示、「ホストの開始を待機」と表示
- 「退出」で初期画面へ戻る

### ルール画面(R1〜R5)

- 起動画面下部の **「ルールを見る」 pill** ボタンと、ロビー画面右上の **「? ルール」 corner バッジ** から呼出
- 5 ページ構成: R1 概要 / R2 目的(4 ブランド)/ R3 ターン 4 ステップ / R4 ハッタリ(2 回まで)/ R5 流札(分配)
- 下部に「戻る / 次へ」ボタン + ページドット、右上に閉じる(×)
- ゲーム中(LISTING/BIDDING/TRANSACTION)は導線非表示
